...
import org.colorcoding.bobas.data.KeyText;
...
public abstract class Measurement<V, U> extends Number implements IMeasurement<V, U>, Comparable<IMeasurement<V, U>> {
			...
			if (characters != null) {
				for (KeyText keyText : characters) {
					if (keyText.key.equalsIgnoreCase(this.getUnit().toString())) {
						return String.format("%s %s", this.getValue(), keyText.text);
					}
				}
			}
		}
		return String.format("%s %s", this.getValue(), this.getUnit());
	}
	 ...
	 * 默认计算值
	 * 
	 * @see org.colorcoding.bobas.data.measurement.IMeasurement#
	 *      toValue()
	 */
	@Override
	public V toValue() {
