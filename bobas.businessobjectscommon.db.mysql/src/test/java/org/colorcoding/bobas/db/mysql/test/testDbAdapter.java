...
import org.colorcoding.bobas.db.mysql.DbAdapter;
...
public class testDbAdapter extends TestCase {
	...
	public void testforDbAdapter() {
		...
		String sql = "SELECT * FROM `AVA_TT_ORDR` WHERE (`DocStatus` = N'P' OR `DocStatus` = N'F') AND `CardCode` IS NOT NULL AND CAST(`DocEntry` AS CHAR) LIKE N'2%' AND `DocEntry` > 2000 AND `DocEntry` <> `B1DocEntry` ORDER BY `DocEntry` DESC, `CardCode` ASC LIMIT 100";
		try {
