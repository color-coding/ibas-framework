...
public class TestBOLogics extends TestCase {
	...
	public void testMaterialsJournal() {
		...
		condition = criteria.getConditions().create();
		condition.setAlias(MaterialsQuantityJournal.PROPERTY_BASEDOCUMENTTYPE.getName());
		condition.setValue(item03.getObjectCode());
		condition = criteria.getConditions().create();
		condition.setAlias(MaterialsQuantityJournal.PROPERTY_BASEDOCUMENTENTRY.getName());
		condition.setValue(item03.getDocEntry());
		condition = criteria.getConditions().create();
		condition.setAlias(MaterialsQuantityJournal.PROPERTY_BASEDOCUMENTLINEID.getName());
		condition.setValue(item03.getLineId());
		operationResult = boRepository.fetchMaterialsQuantityJournal(criteria);
		assertEquals(operationResult.getMessage(), operationResult.getResultCode(), 0);
	...
	public void testRules() {
		...
		assertEquals(Decimal.round(item01.getLineTotal())
				.compareTo(Decimal.round(item01.getQuantity().multiply(item01.getPrice()))) == 0, true);
